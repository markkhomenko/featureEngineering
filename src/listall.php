...
<table class="table">
	...
	$query = db::query("SELECT cid, dateofad, univname, funding, gpa, label, languagescore, subject, noteforchoose, schoolname, subsubject, noteforad FROM `case` WHERE hidden = 0 ORDER BY dateofad DESC");
	...
	?>
		<tr<?php echo (strpos($data['label'], 'REJ') !== false ? " class=\"warning\"" : "") ?>>
			...
			<td><?php echo $_ENV['subject'][$data['subject']] . '-' . $data['subsubject'] ?></td>
